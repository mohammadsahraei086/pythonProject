...
cursor.execute("""DELETE FROM market_watch_data 
                  ...
                  OR market IN ('صندوق قابل معامله', 'حق تقدم بورس' ,'حق تقدم فرابورس', 'حق تقدم پایه', 'پایه');
                  """)
...
cursor.execute("""SELECT ticker FROM market_watch_data
                WHERE (vol_buy_r/NULLIF(no_buy_r,0))/NULLIF((vol_sell_r/NULLIF(no_sell_r,0)),0) >= 1.2
                AND ((vol_buy_r*final)/NULLIF(no_buy_r,0))/10000000 > 20
                AND ((vol_sell_r*final)/NULLIF(no_sell_r,0))/10000000 > 10
                AND ((vol_sell_r*final)/NULLIF(no_sell_r,0))/10000000 < 40
                AND low < day_ul
                AND close != day_ul
